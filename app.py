...
Clean & minimal redesign: no emojis, sidebar navigation
"""
...
<style>
    ...
    h1, h2, h3 { font-weight: 600; letter-spacing: -0.02em; color: #1a1a1a; }
    .section-label { font-size: 0.7rem; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #888; margin-bottom: 0.25rem; }
    .metric-card { background: #f7f7f7; border-radius: 8px; padding: 1.2rem 1.5rem; border: 1px solid #ebebeb; }
    .metric-value { font-size: 2rem; font-weight: 600; color: #1a1a1a; font-family: 'DM Mono', monospace; }
    .metric-label { font-size: 0.8rem; color: #888; margin-top: 0.1rem; }
    .prediction-box { background: #f7f7f7; border-left: 3px solid #1a1a1a; border-radius: 4px; padding: 1rem 1.5rem; margin: 1rem 0; }
    .prediction-box.high { border-left-color: #c0392b; }
    .prediction-box.low { border-left-color: #27ae60; }
    .stButton > button { border-radius: 4px; font-weight: 500; font-size: 0.85rem; border: 1px solid #ddd; background: white; color: #1a1a1a; padding: 0.4rem 1rem; }
    .stButton > button:hover { background: #1a1a1a; color: white; border-color: #1a1a1a; }
...
CREDENTIALS = {"admin": "leadership2024", "demo": "demo123"}
...
def load_data(n_samples=300):
    ...
    return pd.DataFrame({"urgency_score": urgency, "stakeholder_count": stakeholders,
        "data_availability": data_avail, "political_risk": political_risk,
        "resource_cost": resource_cost, "impact": impact})
...
with st.sidebar:
    ...
    page = st.radio("", ["Overview","Distributions","Feature Importance","Decision Profiles","Model Performance","Predict","Export"], label_visibility="collapsed")
...
if page == "Overview":
    ...
    with c1:
        st.markdown(f"<div class='metric-card'><div class='metric-label'>Total Decisions</div><div class='metric-value'>{len(df)}</div></div>", unsafe_allow_html=True)
    with c2:
        st.markdown(f"<div class='metric-card'><div class='metric-label'>High Impact</div><div class='metric-value'>{int(df['impact'].sum())}</div></div>", unsafe_allow_html=True)
    with c3:
        st.markdown(f"<div class='metric-card'><div class='metric-label'>Low Impact</div><div class='metric-value'>{int((df['impact']==0).sum())}</div></div>", unsafe_allow_html=True)
    ...
    fig = px.histogram(df, x=selected, color=df["impact"].map({0:"Low Impact",1:"High Impact"}),
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    fi_df = pd.DataFrame({"Feature":feature_cols,"Importance":clf.feature_importances_}).sort_values("Importance",ascending=True)
    ...
    fig = px.scatter(df, x="urgency_score", y="political_risk", color="profile",
        size="data_availability", hover_data=["stakeholder_count","resource_cost"],
        color_discrete_map={"Profile A":"#2c3e50","Profile B":"#7f8c8d","Profile C":"#bdc3c7"})
    fig.update_layout(plot_bgcolor="white", paper_bgcolor="white", font_family="Inter",
    ...
    st.markdown(f"<div class='prediction-box {box_class}'><div class='section-label'>Prediction</div><div style='font-size:1.6rem;font-weight:600;color:#1a1a1a;margin:0.3rem 0'>{label}</div><div style='font-size:0.85rem;color:#888'>Confidence: <strong style='color:#1a1a1a'>{confidence:.1f}%</strong></div></div>", unsafe_allow_html=True)
    fig = go.Figure(go.Indicator(mode="gauge+number", value=prob[1]*100,
        number={"suffix":"%","font":{"family":"DM Mono","color":"#1a1a1a"}},
        title={"text":"High Impact Probability","font":{"family":"Inter","color":"#888","size":13}},
        gauge={"axis":{"range":[0,100],"tickcolor":"#ccc"},"bar":{"color":accent},
            "bgcolor":"#f7f7f7","bordercolor":"#ebebeb",
            "steps":[{"range":[0,40],"color":"#f7f7f7"},{"range":[40,70],"color":"#f0f0f0"},{"range":[70,100],"color":"#e8e8e8"}]}))
    fig.update_layout(paper_bgcolor="white", font_family="Inter", height=280, margin=dict(l=20,r=20,t=40,b=10))
    st.plotly_chart(fig, use_container_width=True)
    ...
    if st.button("Add to log"):
        st.session_state.prediction_log.append({"urgency_score":urgency,"stakeholder_count":stakeholders,
            "data_availability":data_avail,"political_risk":pol_risk,"resource_cost":res_cost,
            "prediction":label,"confidence_pct":round(confidence,1)})
        st.success("Added.")
    if st.session_state.prediction_log:
        log_df = pd.DataFrame(st.session_state.prediction_log)
        st.dataframe(log_df, use_container_width=True)
        st.download_button("Download log as CSV", log_df.to_csv(index=False).encode("utf-8"), "prediction_log.csv", "text/csv")
    ...
    st.download_button("Download full dataset as CSV", full_df.to_csv(index=False).encode("utf-8"), "full_predictions.csv", "text/csv")
